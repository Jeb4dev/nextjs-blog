...
export default function Card({img, title, date, slug}) {
    return (
        <div className={"drop-shadow shadow-2xl w-full rounded-xl relative"}>
            ...
            <div className={"p-2"}>
                <Link href={ "/posts/" + slug }><h3 className={"font-bold text-xl pb-16 text-black"}>{title}</h3></Link>
                <span className={"absolute bottom-3 right-6 text-[#8A8A8A] font-sm"}><Date dateString={date}/></span>
            </div>
        </div>);
}
